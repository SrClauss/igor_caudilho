...
export default function BateriaPerguntasSF({ onDataChange, prompt, options, scores, perguntas, children, index}) {
    ...
    const handleDataChange = (value, i) => {
        ...
        setData(newPerguntasResult.reduce((acc, curr) => acc + curr, 0))
    }
    return (
        <Card className="p-2 m-2">
            ...
            {perguntas.map((pergunta, index) => (
                <ScoreQuestsSF onDataChange={(value) => handleDataChange(value, index)} index={index} prompt={pergunta} options={options} scores={scores} />
            ))}
